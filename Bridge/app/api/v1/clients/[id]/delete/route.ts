 ...
 *
 * POST /api/v1/clients/[id]/delete
 * Soft-delete con preview/execute para clientes.
 */

import { NextRequest, NextResponse } from "next/server";

import { resolveApiV1RequestContext } from "@/lib/api-v1-context";
import { softDeleteClient } from "@/lib/entity-delete";
...
export async function POST(
  ...
  const previewResult = await softDeleteClient(
    requestContext.tenantId,
    clientId,
    "preview",
    requestedByLabel,
  ...
  if (previewResult.blockedReason) {
    return NextResponse.json(previewResult, { status: 409 });
  }
