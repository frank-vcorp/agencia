...
import { previewDeleteAsset, executeDeleteAsset } from "@/lib/entity-delete";
...
export async function POST(
  ...
  // Preview
  if (mode === "preview") {
    const result = await previewDeleteAsset(tenantId, assetId);
  ...
  if (mode === "execute") {
    ...
    const previewResult = await previewDeleteAsset(tenantId, assetId);
    ...
    const result = await executeDeleteAsset(
      tenantId,
      assetId,
      requestedByLabel,
      approvedByLabel,
